<?php
	...
	$alert='';
		if(empty($_POST['idNote']))
		{
			...
			$query_insert = mysqli_query($conection,"INSERT INTO mantencion(fingreso_mantencion,tipo_mantencion,informe_mantencion,id_equ) 
			                             VALUES ('$fingreso','$tipomantencion','$detalle','$idNote')");
			...
			if($query_insert){
				...
				$sql_est_mant = mysqli_query($conection,"UPDATE equipos 
														 SET mant_equ = '1' 
														 WHERE id_equ ='$idNote'");
				header('location:reporte_mant_proyectores.php');
			}else{
	...
	if(empty($_GET['id']))
	{
		header('location:listado_proyectores.php');
	}
	$idcom = $_GET['id'];
    ...
    WHERE te.id_tipe = 4 AND e.id_equ = $idcom");
...
<head>
	...
	?>
	<title>MANTENCIÓN PROYECTOR</title>
</head>
<body>
<?php include "includes/header.php";	?>
	<section id="container">
		<div class="form_register">
			<h1>Mantención Proyector</h1>
			<hr>
			...
			<form action="" method="POST">
				...
				<label for="serie">Serie Proyector</label>
				...
				<input type="submit" value="REGISTRO MANTENCIÓN" class="btn_grabar">
			</form>
		</div>
	</section>
